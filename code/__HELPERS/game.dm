...
/proc/recursive_organ_check(atom/O)
	...
	while(index <= length(processing_list))
		...
		for(var/atom/B in A)	//objects held within other objects are added to the processing list, unless that object is something that can hold organs safely
			if(!processed_list[B] && !ispath(B, /obj/structure/closet/crate/freezer) && !ispath(B, /obj/structure/closet/secure_closet/freezer)) // APOC EDIT CHANGE // istype() to ispath() to catch subtypes
				processing_list+= B
...
/proc/AnnounceArrival(mob/living/carbon/human/character, rank)
	...
	deadchat_broadcast("<span class='game'> has arrived at the situation at <span class='name'>[A.name]</span>.</span>", "<span class='game'><span class='name'>[character.real_name]</span> ([rank])</span>", follow_target = character, message_type=DEADCHAT_ARRIVALRATTLE)
